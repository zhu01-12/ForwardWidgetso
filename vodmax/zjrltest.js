...
function buildItem(item, mediaType, { rank, customSub, descPrefix, weekdayName } = {}) {
    const dateStr = item.first_air_date || item.release_date || "";
    const year = dateStr.substring(0, 4);
    
    // 处理类型：如果是动漫模块，且没有 TMDB 数据，默认为动画
    ...
    return {
        ...
        // --- 核心优化：如果是动漫，显示“周一 · 动画”；普通剧集显示类型 ---
        genreTitle: weekdayName ? `${weekdayName} · ${genres}` : genres,
        
        // --- 核心优化：竖版副标题显示具体日期 ---
        description: dateStr || item.overview || "暂无简介",
        
        subTitle: customSub || item.original_name,
        posterPath: item.poster_path, // buildItem 内部由内核处理前缀，所以只传 path 或完整 url
...
async function loadBangumiCalendar(params = {}) {
    ...
    const dayName = {1:"周一",2:"周二",3:"周三",4:"周四",5:"周五",6:"周六",7:"周日"}[targetDayId];
...
async function loadTvCalendar(params = {}) {
    ...
    try {
        ...
        return (res.results || []).map(item => buildItem(item, "tv", {
            customSub: mode === "update_today" ? "🆕 今日更新" : `📅 ${item[dateField]?.slice(5)}`
        }));
    } catch (e) { return []; }
}
...
async function loadVarietyCalendar(params = {}) {
    ...
    try {
        ...
        const promises = items.map(async (item) => {
            ...
            const d = await Widget.tmdb.get(`/tv/${item.show.ids.tmdb}`, { params: { language: "zh-CN" } });
            if (!d) return null;
            const ep = item.episode;
            return buildItem(d, "tv", { customSub: `S${ep.season}E${ep.number} · ${ep.title || "最新集"}` });
        });
        return (await Promise.all(promises)).filter(Boolean);
    } catch (e) {
...
async function fetchTmdbVariety(region, dateStr) {
    const q = { language: "zh-CN", sort_by: "popularity.desc", with_genres: "10764|10767" };
    ...
    try {
        ...
        return (res.results || []).map(item => buildItem(item, "tv", { customSub: dateStr ? `📅 ${dateStr}` : "近期热播" }));
    } catch (e) { return []; }
}
...
async function searchTmdbBestMatch(q1, q2) {
    ...
    return (await search(q1)) || (await search(q2));
}
